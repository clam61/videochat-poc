...
const connectSignaling = () => {
  ...
  signaling.on("close", () => {
    console.warn("[signal] closed, reconnecting...");
    setTimeout(connectSignaling, 2000);
  });
  ...
  pc.ontrack = (ev) => {
    ...
    sink.ondata = (d) => {
      ...
      if (d.samples instanceof Float32Array) {
        ...
        int16Buffer = Buffer.from(tmp.buffer);
      } else int16Buffer = Buffer.from(d.samples.buffer);
      const sr = d.sampleRate || 48000;
      if (sr !== 16000) {
        const down = simpleDownsampleInt16(new Int16Array(int16Buffer.buffer), sr, 16000);
        pcmStream.write(Buffer.from(down.buffer));
      } else pcmStream.write(int16Buffer);
    };
    track.onended = () => {
      try {
        sink.stop();
    ...
    startAwsTranscribePipeline(pcmStream, clientId).catch(console.error);
  };
...
// --- Simple downsample ---
function simpleDownsampleInt16(input, inRate, outRate) {
  if (outRate === inRate) return input;
  const ratio = inRate / outRate,
    newLength = Math.round(input.length / ratio);
  const out = new Int16Array(newLength);
  let offsetRes = 0,
    offsetBuf = 0;
  while (offsetRes < newLength) {
    ...
    let acc = 0,
      count = 0;
    for (let i = offsetBuf; i < next && i < input.length; i++) {
      acc += input[i];
      count++;
    }
    out[offsetRes] = count ? Math.round(acc / count) : 0;
    offsetRes++;
    offsetBuf = next;
  }
  return out;
...
// --- Stream audio from Polly + translate ---
async function startAwsTranscribePipeline(pcmStream, clientId) {
  const language = languages.get(clientId) || "en-US";
  let bufferText = "",
    debounce = null,
  ...
  const pushSynth = async (buf) => {
    if (!buf || buf.length === 0) return;
    synthQueue.push(buf);
    if (!feeding) {
      feeding = true;
      while (synthQueue.length > 0) {
        const b = synthQueue.shift();
        const audioSource = audioSources.get(clientId);
        if (audioSource) feedAudioBufferToSource(audioSource, b);
  ...
  const translateAndSynthesize = async (txt) => {
    ...
    try {
      ...
      const voice = tgt === "es" ? "Lucia" : "Joanna";
      const synth = await pollyClient.send(
        new SynthesizeSpeechCommand({
          OutputFormat: "pcm",
          Text: translated,
      ...
      // const audioBuf = Buffer.from(synth.AudioStream);
      await pushSynth(audioBuf);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const flushBuffer = async () => {
    ...
    bufferText = "";
    if (txt) await translateAndSynthesize(txt);
  };
  ...
  const command = new StartStreamTranscriptionCommand({
    ...
    MediaSampleRateHertz: 16000,
    AudioStream: (async function* () {
      for await (const c of pcmStream) yield { AudioEvent: { AudioChunk: c } };
    })(),
  });
  const resp = await transcribeClient.send(command);
          ...
          for (const r of ev.TranscriptEvent.Transcript.Results) {
            ...
            if (r.IsPartial) {
              ...
              debounce = setTimeout(async () => {
                ...
                bufferText = "";
                if (t) await translateAndSynthesize(t);
              }, 350);
            } else {
              if (debounce) {
                clearTimeout(debounce);
              ...
              const t = (bufferText ? bufferText + " " + text : text).trim();
              bufferText = "";
              if (t) await translateAndSynthesize(t);
            }
          }
