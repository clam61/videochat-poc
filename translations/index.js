...
import avahqWrtc, { MediaStream } from "@avahq/wrtc";
import { PollyClient, SynthesizeSpeechCommand } from "@aws-sdk/client-polly";
import {
  StartStreamTranscriptionCommand,
  TranscribeStreamingClient,
...
const { RTCPeerConnection, nonstandard } = avahqWrtc;
const { RTCAudioSink, RTCAudioSource } = nonstandard;
...
const connectSignaling = () => {
  ...
  signaling.on("close", () => setTimeout(connectSignaling, 2000));
  signaling.on("error", (e) => {
    console.error("[signal] error", e);
    try {
      signaling.close();
...
async function handleOffer(clientId, offerSdp) {
  ...
  audioSources.set(clientId, audioSource);
  const ttsTrack = audioSource.createTrack();
  const ttsStream = new MediaStream();
  ...
  pc.onicecandidate = (e) => {
    if (e.candidate && signaling?.readyState === WebSocket.OPEN) {
      signaling.send(
        JSON.stringify({
          type: "ice-candidate",
          candidate: e.candidate,
  ...
  pc.onconnectionstatechange = () => {
    if (["closed", "failed", "disconnected"].includes(pc.connectionState)) cleanup();
  };

  // When we receive audio from a client, we start transcription.
  pc.ontrack = (ev) => {
    ...
    sink.ondata = (d) => {
      const int16 = d.samples instanceof Float32Array ? floatTo16BitPCM(d.samples) : d.samples;
      pcmStream.write(Buffer.from(int16.buffer));
    };
    ...
    startAwsTranscribePipeline(pcmStream, clientId, audioSource).catch(console.error);
  };
  ...
  signaling?.send(
    JSON.stringify({ type: "answer", sdp: answer.sdp, from: "translation-server", to: clientId })
  );

  return { pc, audioSource };
}

async function sendTtsToClient(clientId, text) {
  const audioSource = audioSources.get(clientId);
  ...
  const buffer = await streamToBuffer(synth.AudioStream);
  feedAudioBufferToSource(audioSource, buffer);
}

async function startAwsTranscribePipeline(pcmStream, clientId, audioSource) {
  const language = languages.get(clientId) || "en-US";
  const command = new StartStreamTranscriptionCommand({
    LanguageCode: language,
    MediaEncoding: "pcm",
    MediaSampleRateHertz: 16000,
    AudioStream: (async function* () {
      for await (const chunk of pcmStream) yield { AudioEvent: { AudioChunk: chunk } };
    })(),
  });
  ...
  for await (const event of response.TranscriptResultStream) {
    ...
    for (const res of event.TranscriptEvent.Transcript.Results) {
      ...
      const isFinal = !res.IsPartial;
      if (!isFinal && text.split(" ").length < 3) continue;

      const srcLangCode = language.split("-")[0];
      const targetLangCode = srcLangCode === "en" ? "es" : "en";
      ...
      // Sending TTS via RTCAudioSource
      await sendTtsToClient(clientId, translatedText);
    }
  }
}
...
function feedAudioBufferToSource(audioSource, buffer) {
  ...
  let combined = Buffer.concat([audioSource._leftover, buffer]);
  const frames = Math.floor(combined.length / BYTES_PER_FRAME);
  const used = frames * BYTES_PER_FRAME;
  audioSource._leftover = combined.subarray(used);

  for (let i = 0; i < frames; i++) {
    const frame = combined.subarray(i * BYTES_PER_FRAME, i * BYTES_PER_FRAME + BYTES_PER_FRAME);
    const int16 = new Int16Array(BYTES_PER_FRAME / 2);
    for (let j = 0; j < int16.length; j++) int16[j] = frame.readInt16LE(j * 2);
...
function floatTo16BitPCM(float32Array) {
  const tmp = new Int16Array(float32Array.length);
...
//     // Spawn the transcribe->translate->polly pipeline (improved streaming)
//     startAwsTranscribePipeline(pcmStream, clientId).catch((err) =>
//       console.error("transcribe pipeline error", err)
//     );
//   };
...
// // ----------------- Improved streaming pipeline -----------------
// async function startAwsTranscribePipeline(pcmStream, clientId) {
//   const language = languages.get(clientId) || "en-US";
//   console.log(`[transcribe] start for ${clientId} lang=${language}`);

//   // Streaming control variables
//   let bufferText = ""; // latest partial text
//   let debounceTimer = null;
//   const DEBOUNCE_MS = 350; // tune: lower -> less latency, higher -> fewer partial fragments
//   let synthQueue = [];
//   let isFeeding = false;
//   const MAX_QUEUE_LENGTH = 10; // safety cap to avoid OOM in bad network

//   // sequentially feed PCM buffers from synthQueue into RTCAudioSource
//   const pushSynthBuffer = async (audioBuf) => {
//     if (!audioBuf || audioBuf.length === 0) return;
//     // safety cap
//     if (synthQueue.length > MAX_QUEUE_LENGTH) {
//       console.warn(`[synthQueue] queue too long (${synthQueue.length}), dropping oldest`);
//       synthQueue.shift();
//     }
//     synthQueue.push(audioBuf);
//     if (!isFeeding) {
//       isFeeding = true;
//       try {
//         while (synthQueue.length > 0) {
//           const buf = synthQueue.shift();
//           const audioSource = audioSources.get(clientId);
//           if (!audioSource) {
//             console.warn(`[synthQueue] no audioSource for ${clientId}, dropping buffer`);
//             continue;
//           }
//           feedAudioBufferToSource(audioSource, buf);
//           // yield to event loop so audioSource.onData can be processed
...
//   // Translate + Polly synth, then push to queue
//   const translateAndSynthesize = async (textToTranslate) => {
...
//       );
//       const translatedText = tr.TranslatedText;
//       console.log(`[translate][${clientId}] ->`, translatedText);

//       // Send translated text to signaling server (UI preview)
//       if (signaling && signaling.readyState === WebSocket.OPEN) {
//         signaling.send(
//           JSON.stringify({
//             type: "translation-text",
...
//       // Convert to Buffer and push to queue
//       const audioBuf = await streamToBuffer(synth.AudioStream);
//       if (audioBuf && audioBuf.length > 0) {
//         await pushSynthBuffer(audioBuf);
//       }
//     } catch (err) {
//       console.error("[translateAndSynthesize] error:", err);
//     }
//   };
...
//     bufferText = "";
//     if (text !== "") {
//       await translateAndSynthesize(text).catch((e) =>
//         console.error("[flushBufferNow] translate/synth err", e)
//       );
//     }
//   };

//   // Build transcribe command streaming audio from pcmStream
//   const command = new StartStreamTranscriptionCommand({
//     LanguageCode: language,
//     MediaEncoding: "pcm",
//     MediaSampleRateHertz: 16000,
...
//   // Process transcript stream: use partials + debounce for low latency
//   (async () => {
//     try {
//       for await (const event of resp.TranscriptResultStream) {
//         if (event.TranscriptEvent) {
//           for (const r of event.TranscriptEvent.Transcript.Results) {
//             if (!r || !r.Alternatives || !r.Alternatives[0]) continue;
//             const text = r.Alternatives[0].Transcript || "";

//             if (r.IsPartial) {
//               // update latest partial and restart debounce
//               bufferText = text;
//               if (debounceTimer) clearTimeout(debounceTimer);
//               debounceTimer = setTimeout(async () => {
//                 debounceTimer = null;
//                 const toSynth = bufferText.trim();
//                 bufferText = "";
//                 if (toSynth) {
//                   translateAndSynthesize(toSynth).catch((e) =>
//                     console.error("[debounce translateAndSynthesize] err", e)
//                   );
//                 }
//               }, DEBOUNCE_MS);
//             } else {
//               // final result: cancel pending debounce and synth final+buffer
//               const finalText = text.trim();
//               if (debounceTimer) {
//                 clearTimeout(debounceTimer);
//                 debounceTimer = null;
//               }
//               const toSynth = (bufferText ? bufferText + " " + finalText : finalText).trim();
//               bufferText = "";
//               if (toSynth) {
//                 await translateAndSynthesize(toSynth).catch((e) =>
//                   console.error("[final translateAndSynthesize] err", e)
//                 );
//               }
//             }
//           }
...
//       await flushBufferNow();
//     } catch (err) {
//       console.error("[transcribe stream loop] error", err);
//     }
//   })();
// }
...
// // start
// connectSignaling();
// console.log("translation-server running and waiting for offers...");
