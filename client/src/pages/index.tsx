...
export default function Home() {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState<{ type: 'none' } | { type: 'source', language: 'es-MX' | "en-US" }>({ type: 'none' });
  ...
  useEffect(() => {
    ...
    const initMedia = async () => {
      ...
      pcVideo.current.ontrack = (e) => { if (remoteVideo.current) remoteVideo.current.srcObject = e.streams[0]; };
      ...
      localStream.current.getAudioTracks().forEach((t) => pcAudio.current?.addTrack(t, localStream.current!));

      // When remote track is received, set it to remoteAudio element
      pcAudio.current.ontrack = (e) => { if (remoteAudio.current) remoteAudio.current.srcObject = e.streams[0]; };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "flex", gap: "1rem", marginTop: "1rem" }}>
        <div>
          <h3>Local Video</h3>
          <video ref={localVideo} autoPlay muted playsInline style={{ width: 300, height: 200, backgroundColor: "black" }} />
        </div>
        <div>
          <h3>Remote Video</h3>
          <video ref={remoteVideo} autoPlay playsInline style={{ width: 300, height: 200, backgroundColor: "black" }} />
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <div className="relative inline-block text-left">
          <div>
            <button onClick={toggleDropdown} className="inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50">
              {selectedLanguage.type === 'none' ? 'Languages' : selectedLanguage.language}
            </button>
          </div>
          <div ref={dropDownRef} className="absolute left-0 z-10 mt-2 w-32 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5" style={{ display: "none" }}>
            <div className="py-1">
              <button onClick={() => handleLanguageSelection("en-US")} className="block px-4 py-2 w-full text-sm text-gray-700 hover:bg-gray-100">en-US</button>
              <button onClick={() => handleLanguageSelection("es-MX")} className="block px-4 py-2 w-full text-sm text-gray-700 hover:bg-gray-100">es-MX</button>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className={`${isTranslationActive ? "bg-red-900" : "bg-green-700"} text-white font-bold py-2 px-4 rounded transition-colors duration-300`}
        >
